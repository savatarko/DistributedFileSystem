		...
		while (working) {
			try {
				...
				if (message.equals("Hail")) {
					...
					ServentInfo serventInfo = new ServentInfo(newServentIp, newServentPort);
					
					if (activeServents.size() == 0) {
						socketWriter.write(String.valueOf(-1) + "\n");
						activeServents.add(serventInfo); //first one doesn't need to confirm
					} else {
					...
					ServentInfo serventInfo = new ServentInfo(newServentIp, newServentPort);
					if(!activeServents.contains(serventInfo)) {
						activeServents.add(serventInfo);
					}
					newServentSocket.close();
				}
					...
					activeServents.remove(new ServentInfo(serventIp, serventPort));
				}
